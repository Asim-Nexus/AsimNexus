...
- /api/v1/user/consensus/* - User-initiated proposals
"""
...
async def get_user_profile(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        profile = {"name": "Unknown", "email": "unknown@example.com"}
...
async def assign_twin_task(request: TwinTaskRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        "task": request.task,
        "status": "assigned",
        "twin_status": twin.status() if twin else {"status": "mock"}
    }
...
# Grievance endpoints
def route_grievance(category: str) -> str:
...
async def submit_grievance(request: GrievanceRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        "target_agency": route_grievance(request.category)
    }
...
# Consensus endpoints (user-initiated proposals)
...
async def create_user_proposal(
    ...
    """Create a user proposal (lower threshold)"""
    ...
    return {
        ...
        "sector": "user",
        "status": "created",
        "threshold": 0.33  # Lower threshold for user proposals
    }
...
async def citizen_api_status() -> Dict[str, Any]:
    ...
    return {
        "mode": "citizen",
        "model": "local_first",
        "modules": ["profile", "twin", "grievance", "mesh", "consensus"],
        "status": "operational"
    }
