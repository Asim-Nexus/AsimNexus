...
from core.orchestrator.tools.sandbox.docker_sandbox import DockerSandbox, docker_sandbox
...
class OSToolExecutor:
    """
    Main orchestrator for AsimNexus tool execution.
    ...
    def __init__(
        ...
        self.audit_path = audit_path or Path("data/asim_tools_audit.jsonl")
        ...
        logger.info(
            "🌐 OSToolExecutor initialized — Capability-Gated AsimNexus Tools active"
        )
    ...
    async def execute(
        ...
        if reg is None:
            return self._denied_response(
                ...
                f"Tool '{tool_name}' not registered in AsimNexus Tool Registry",
                t_start,
                "unknown",
            )
