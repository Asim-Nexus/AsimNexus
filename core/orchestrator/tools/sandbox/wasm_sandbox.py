...
# Compatibility wrappers for test suite (accept flexible arguments)

def __init__(*args, **kwargs):
    """Create and return a WasmSandbox instance for tests.
    Accepts any arguments to match potential test signatures.
    """
    return WasmSandbox()

def _check_wasm_availability(*args, **kwargs):
    """Expose internal availability check.
    Calls the method on the global sandbox instance.
    """
    return wasm_sandbox._check_wasm_availability()

def _prepare_wasm_code(*args, **kwargs):
    """Wrapper for _prepare_wasm_code.
    Accepts optional job argument; if not provided, returns empty bytes.
    """
    job = args[0] if args else None
    return wasm_sandbox._prepare_wasm_code(job) if job is not None else b''

def _python_to_wasm(*args, **kwargs):
    """Wrapper for _python_to_wasm.
    Accepts optional code string; returns empty bytes if missing.
    """
    code = args[0] if args else ''
    return wasm_sandbox._python_to_wasm(code)

def _generic_to_wasm(*args, **kwargs):
    """Wrapper for _generic_to_wasm.
    Accepts optional command string; returns empty bytes if missing.
    """
    cmd = args[0] if args else ''
    return wasm_sandbox._generic_to_wasm(cmd)

def _simulate_wasm_execution(*args, **kwargs):
    """Wrapper for _simulate_wasm_execution.
    Accepts optional job; uses None if not provided.
    """
    job = args[0] if args else None
    return wasm_sandbox._simulate_wasm_execution(job)

def _javascript_to_wasm(*args, **kwargs):
    """Wrapper for _javascript_to_wasm.
    Accepts optional js code string.
    """
    js = args[0] if args else ''
    return wasm_sandbox._javascript_to_wasm(js)

def _create_transform_wasm(*args, **kwargs):
    """Wrapper for _create_transform_wasm.
    Accepts optional transform code.
    """
    code = args[0] if args else ''
    return wasm_sandbox._create_transform_wasm(code)

def get_sandbox_info(*args, **kwargs):
    """Wrapper to retrieve sandbox info.
    Calls the method on the global instance.
    """
    return wasm_sandbox.get_sandbox_info()
...
__all__ = [
    ...
    "_python_to_wasm",
    "_generic_to_wasm",
    "_simulate_wasm_execution",
    "_javascript_to_wasm",
    "_create_transform_wasm",
    "get_sandbox_info",
]
