...
if TYPE_CHECKING:
    from core.mesh.p2p_transport import P2PTransport, P2PMessage, PeerInfo
else:
...
# ─── Hybrid Logical Clock (DDIA Chapter 8: Clocks & Network Issues) ──────────
#
# HLC combines physical time (wall clock) with a logical counter to provide
# causally-consistent timestamps even when wall clocks drift or jump backwards
# (e.g., due to NTP sync).
#
# Reference: DDIA Chapter 8 ("Unreliable Clocks"), Sandeep Kulkarni's HLC paper.
#
# Properties:
#   1. HLC time always >= physical time (monotonicity)
#   2. HLC time is comparable across nodes (causality)
#   3. If event A happens-before event B, then HLC(A) < HLC(B)
...
class HLCTimestamp:
    ...
    Attributes:
        physical_time: Wall clock in milliseconds (int, not float — avoids drift issues).
        logical_counter: Monotonic counter for events within same physical ms.
        node_id: Originating node ID (for tie-breaking).
    """
    physical_time: int
    logical_counter: int
    node_id: str = ""
    ...
    def __lt__(self, other: "HLCTimestamp") -> bool:
        """Causality comparison: physical first, then logical, then node_id."""
...
class HybridLogicalClock:
    ...
    Generates monotonic, causally-consistent timestamps that combine
    physical time with a logical counter.
    
    Usage:
        hlc = HybridLogicalClock(node_id="node_abc")
        ts1 = hlc.generate()  # New timestamp for a local event
        hlc.update(ts2)       # Update clock upon receiving remote timestamp
    """
    ...
    def __init__(self, node_id: str = "unknown"):
        ...
        self._latest = HLCTimestamp(physical_time=now, logical_counter=0, node_id=node_id)

    def generate(self) -> HLCTimestamp:
        """
        Generate a new HLC timestamp for a local event.
        
        Returns:
            HLCTimestamp that is guaranteed to be > any previously generated
            or received timestamp (monotonic + causal).
        """
        with self._lock:
            physical_now = int(time.time() * 1000)

            if physical_now > self._latest.physical_time:
                # Physical clock moved forward — reset logical counter
                self._latest = HLCTimestamp(
                    physical_time=physical_now,
                    logical_counter=0,
                    node_id=self.node_id,
                )
            else:
                ...
                self._latest = HLCTimestamp(
                    physical_time=self._latest.physical_time,
                    logical_counter=self._latest.logical_counter + 1,
                    node_id=self.node_id,
                )

            return HLCTimestamp(
                physical_time=self._latest.physical_time,
                logical_counter=self._latest.logical_counter,
                node_id=self.node_id,
            )

    def update(self, remote_ts: HLCTimestamp) -> HLCTimestamp:
        """
        Update clock upon receiving a remote timestamp.
        
        Implements the HLC update rule from DDIA Chapter 8:
            new_physical = max(physical_now, local_physical, remote_physical)
            if new_physical == local_physical == remote_physical:
                logical = max(local_logical, remote_logical) + 1
            elif new_physical == local_physical:
        ...
        Args:
            remote_ts: The timestamp received from a remote node.
            
        Returns:
            The updated local timestamp.
        """
        with self._lock:
            physical_now = int(time.time() * 1000)
            max_physical = max(physical_now, self._latest.physical_time, remote_ts.physical_time)

            if max_physical == self._latest.physical_time == remote_ts.physical_time:
                logical = max(self._latest.logical_counter, remote_ts.logical_counter) + 1
            elif max_physical == self._latest.physical_time:
                logical = self._latest.logical_counter + 1
            elif max_physical == remote_ts.physical_time:
                logical = remote_ts.logical_counter + 1
            else:
                logical = 0
    ...
    def get_current(self) -> HLCTimestamp:
        """Get the current HLC timestamp without advancing the clock."""
        with self._lock:
            return HLCTimestamp(
                physical_time=self._latest.physical_time,
                logical_counter=self._latest.logical_counter,
...
# ─── Global HLC instance ─────────────────────────────────────────────────────
_HLC_INSTANCE: Optional[HybridLogicalClock] = None
_HLC_LOCK = threading.Lock()


def get_hlc(node_id: Optional[str] = None) -> HybridLogicalClock:
    """Get or create the global HybridLogicalClock singleton."""
    global _HLC_INSTANCE
    if _HLC_INSTANCE is None:
        with _HLC_LOCK:
            if _HLC_INSTANCE is None:
                nid = node_id or os.getenv("ASIM_NODE_ID", "crdt_hlc")
                _HLC_INSTANCE = HybridLogicalClock(node_id=nid)
    return _HLC_INSTANCE


def reset_hlc() -> None:
    """Reset the HLC singleton (for testing)."""
    global _HLC_INSTANCE
    with _HLC_LOCK:
        _HLC_INSTANCE = None
...
class CRDTOperation:
    ...
    timestamp: HLCTimestamp = field(default_factory=lambda: get_hlc().generate())
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CRDTOperation':
        ...
        if isinstance(ts_data, dict):
            timestamp = HLCTimestamp.from_dict(ts_data)
        elif isinstance(ts_data, (int, float)):
            # Backwards compatibility: old float timestamps
            timestamp = HLCTimestamp(physical_time=int(ts_data * 1000), logical_counter=0, node_id=data.get("node_id", ""))
        else:
            timestamp = get_hlc().generate()
        return cls(
            id=data["id"],
...
class GCounter:
    ...
    def _make_op_id(self, node_id: str) -> str:
        """Generate a unique operation ID using HLC timestamp."""
        hlc_ts = get_hlc().generate()
        return hashlib.sha256(f"{node_id}{hlc_ts.physical_time}{hlc_ts.logical_counter}".encode()).hexdigest()[:16]
...
class GMap:
    ...
    def _make_op_id(self, node_id: str, key: str = "") -> str:
        """Generate a unique operation ID using HLC timestamp."""
        hlc_ts = get_hlc().generate()
        return hashlib.sha256(f"{node_id}{key}{hlc_ts.physical_time}{hlc_ts.logical_counter}".encode()).hexdigest()[:16]
    ...
    def put(self, key: str, value: Any, node_id: str) -> CRDTOperation:
        ...
        self.data[key] = value
        op = CRDTOperation(
            id=self._make_op_id(node_id, key),
            crdt_id=self.crdt_id,
            crdt_type=self.crdt_type,
            operation="put",
            key=key,
...
class LWWMap:
    ...
    def put(self, key: str, value: Any, node_id: str) -> CRDTOperation:
        ...
        hlc_ts = get_hlc().generate()
        self.data[key] = (value, hlc_ts, node_id)
        op = CRDTOperation(
            id=self._make_op_id(node_id, key),
            crdt_id=self.crdt_id,
            crdt_type=self.crdt_type,
...
class LWWRegister:
    ...
    def __init__(self, crdt_id: str):
        ...
        self.timestamp: HLCTimestamp = HLCTimestamp(physical_time=0, logical_counter=0, node_id="")
    ...
    def merge(self, other: 'LWWRegister') -> List[CRDTOperation]:
        ...
        if other.timestamp > self.timestamp:
            ...
            return [CRDTOperation(
                id=self._make_op_id(other.node_id or "unknown"),
                crdt_id=self.crdt_id,
                crdt_type=self.crdt_type,
                operation="merge",
                value=other.value,
...
class CRDTStore:
    ...
    async def start(self, transport: Optional['P2PTransport'] = None):
        ...
        from core.mesh.p2p_transport import WSMessageType
    ...
    async def _handle_sync_request(self, msg: 'P2PMessage'):
        ...
        from core.mesh.p2p_transport import P2PMessage
    ...
    def get_pending_operations(self, since: Optional[float] = None) -> List[CRDTOperation]:
        ...
        # Convert float since to HLCTimestamp for comparison
        since_hlc = HLCTimestamp(physical_time=int(since * 1000), logical_counter=0, node_id="")
        return [op for op in self.pending_operations if op.timestamp > since_hlc]
        ...
        for crdt_id, crdt in self.crdts.items():
            if isinstance(crdt, GCounter):
                ...
                state[crdt_id] = {
                    "type": "lww_register",
                    "value": crdt.get(),
                    "timestamp": crdt.timestamp.to_dict(),
                ...
                state[crdt_id] = {
                    "type": "lww_map",
                    "data": {k: {"value": v[0], "timestamp": v[1].to_dict(), "node_id": v[2]} for k, v in crdt.data.items()},
                }
    ...
    def get_sync_state(self, since: float = 0) -> Dict[str, Any]:
        ...
        return {
            "node_id": self.node_id,
            "operations": [op.to_dict() for op in self.operation_log if op.timestamp > since_hlc],
        ...
        for crdt_id, crdt_data in remote_crdt_state.items():
            ...
            try:
                if crdt_type == "g_counter" and isinstance(local_crdt, GCounter):
                    ...
                    ts_data = crdt_data.get("timestamp")
                    if isinstance(ts_data, dict):
                        remote.timestamp = HLCTimestamp.from_dict(ts_data)
                    elif isinstance(ts_data, (int, float)):
                        # Backwards compatibility: old float timestamps
                        remote.timestamp = HLCTimestamp(physical_time=int(ts_data * 1000), logical_counter=0, node_id="")
                    else:
                        remote.timestamp = HLCTimestamp(physical_time=0, logical_counter=0, node_id="")
                    ...
                    if isinstance(remote_data, dict):
                        for k, v in remote_data.items():
                            ts_data = v["timestamp"]
                            if isinstance(ts_data, dict):
                                ts = HLCTimestamp.from_dict(ts_data)
                            elif isinstance(ts_data, (int, float)):
                                ts = HLCTimestamp(physical_time=int(ts_data * 1000), logical_counter=0, node_id="")
                            else:
                                ts = HLCTimestamp(physical_time=0, logical_counter=0, node_id="")
                            remote.data[k] = (v["value"], ts, v.get("node_id", "remote"))
                    new_ops = local_crdt.merge(remote)
                    applied_count += len(new_ops)
    ...
    def cleanup_old_operations(self, max_age: Optional[float] = None):
        ...
        cutoff_float = time.time() - age
        cutoff = HLCTimestamp(physical_time=int(cutoff_float * 1000), logical_counter=0, node_id="")
