...
_LOAD_SHED_CAPACITY = int(os.getenv("ASIM_LOAD_SHED_CAPACITY", "100"))
_LOAD_SHED_REFILL_RATE = float(os.getenv("ASIM_LOAD_SHED_REFILL_RATE", "10.0"))
_LOAD_SHED_MAX_LATENCY_MS = float(os.getenv("ASIM_LOAD_SHED_MAX_LATENCY_MS", "500.0"))
_LOAD_SHED_MIN_BANDWIDTH_KBPS = float(os.getenv("ASIM_LOAD_SHED_MIN_BANDWIDTH_KBPS", "100.0"))


class TokenBucketLoadShedder:
    """
    Token Bucket algorithm for load shedding (DDIA Chapter 4: Scalability).
    
    Prevents system overload by:
    1. Dropping requests when latency exceeds threshold
    2. Rate-limiting via token bucket when bandwidth is low
    3. Providing backpressure to upstream callers
    
    Reference: System Design Interview (Alex Xu), Google SRE Book.
    """

    def __init__(self, name: str = "default",
                 capacity: int = _LOAD_SHED_CAPACITY,
                 refill_rate: float = _LOAD_SHED_REFILL_RATE,
                 max_allowed_latency_ms: float = _LOAD_SHED_MAX_LATENCY_MS,
                 min_bandwidth_kbps: float = _LOAD_SHED_MIN_BANDWIDTH_KBPS):
        self.name = name
    ...
    def allow_request(self, current_latency_ms: float,
                      current_bandwidth_kbps: float) -> bool:
        """
        Check if a request should be allowed or shed.
        
        Args:
            current_latency_ms: Current observed latency for the mesh.
            current_bandwidth_kbps: Current observed bandwidth.
            
        Returns:
            True if request is allowed, False if it should be dropped (shed).
        """
        with self._lock:
            # ─── Latency-based shedding ───────────────────────────────────────
            # If latency exceeds threshold, shed aggressively
            if current_latency_ms > self.max_allowed_latency_ms:
                self.dropped_count += 1
                logger.warning(
                    f"[LOAD_SHED:{self.name}] Dropped — latency "
                    f"{current_latency_ms:.0f}ms > {self.max_allowed_latency_ms:.0f}ms"
                )
                return False

            # ─── Bandwidth-based shedding ─────────────────────────────────────
            if current_bandwidth_kbps < self.min_bandwidth_kbps:
                self.dropped_count += 1
                logger.warning(
                    f"[LOAD_SHED:{self.name}] Dropped — bandwidth "
                    f"{current_bandwidth_kbps:.0f}kbps < {self.min_bandwidth_kbps:.0f}kbps"
                )
                return False
            ...
            self.dropped_count += 1
            logger.warning(
                f"[LOAD_SHED:{self.name}] Dropped — rate limit exceeded "
                f"(tokens={self.tokens:.1f})"
            )
            return False

    def get_stats(self) -> Dict[str, Any]:
        """Get load shedder statistics for monitoring."""
        with self._lock:
            total = self.allowed_count + self.dropped_count
            shed_rate = (self.dropped_count / total * 100) if total > 0 else 0.0
            return {
                "name": self.name,
                ...
                "shed_rate_percent": round(shed_rate, 1),
...
class MultiMeshRouter:
    ...
    def __init__(self, single_machine: Optional[bool] = None):
        ...
        # ─── Load Shedder (DDIA Chapter 4: Scalability) ──────────────────────
        # Token bucket rate limiter that drops requests when latency is high
        # or bandwidth is low. Provides backpressure to upstream callers.
        self._load_shedder = TokenBucketLoadShedder(
            name="multi-mesh-router",
            capacity=_LOAD_SHED_CAPACITY,
            refill_rate=_LOAD_SHED_REFILL_RATE,
            max_allowed_latency_ms=_LOAD_SHED_MAX_LATENCY_MS,
            min_bandwidth_kbps=_LOAD_SHED_MIN_BANDWIDTH_KBPS,
        )
    ...
    def select_mesh(self, requirements: Optional[MeshRequirements] = None) -> Tuple[MeshType, MeshProfile]:
        ...
        Evaluates in order:
          ...
          4. Load shedding check (DDIA Chapter 4) — drops if latency/bandwidth degraded
          5. Priority order (lower = better)
        ...
        Raises:
            RuntimeError: If no mesh meets the requirements or load shedding drops all.
        """
        ...
        with self._lock:
            ...
            # ─── Load Shedding Check (DDIA Chapter 4: Scalability) ───────────
            # Check the best candidate against load shedder thresholds.
            # If the best mesh is degraded, try the next best, or shed.
            best_mtype, best_profile = candidates[0]
            if not self._load_shedder.allow_request(
                current_latency_ms=best_profile.avg_latency_ms,
                current_bandwidth_kbps=best_profile.bandwidth_kbps,
            ):
                # Best mesh was shed — try next best candidates
                for alt_mtype, alt_profile in candidates[1:]:
                    if self._load_shedder.allow_request(
                        current_latency_ms=alt_profile.avg_latency_ms,
                        current_bandwidth_kbps=alt_profile.bandwidth_kbps,
                    ):
                        logger.warning(
                            f"⚠️ Load shedding: {best_mtype.value} shed, "
                            f"falling back to {alt_mtype.value}"
                        )
                        return (alt_mtype, alt_profile)

                # All candidates shed — raise error with backpressure signal
                raise RuntimeError(
                    f"Load shedding: all meshes degraded "
                    f"(latency={best_profile.avg_latency_ms:.0f}ms, "
                    f"bandwidth={best_profile.bandwidth_kbps:.0f}kbps)"
                )
    ...
    def route_through_mesh(self, mesh: MeshType, data: bytes,
        ...
        Includes load shedding check (DDIA Chapter 4) — if the mesh is
        degraded (high latency / low bandwidth), the request is shed
        ...
        with self._lock:
            ...
            # ─── Load Shedding Check (DDIA Chapter 4: Scalability) ───────────
            if not self._load_shedder.allow_request(
                current_latency_ms=profile.avg_latency_ms,
                current_bandwidth_kbps=profile.bandwidth_kbps,
            ):
                logger.warning(
                    f"⛔ Load shedding: routing through {mesh.value} denied "
                    f"(latency={profile.avg_latency_ms:.0f}ms, "
                    f"bandwidth={profile.bandwidth_kbps:.0f}kbps)"
                )
                return False
    ...
    def get_load_shedder_stats(self) -> Dict[str, Any]:
        """Get load shedder statistics for monitoring (DDIA Chapter 4)."""
        return self._load_shedder.get_stats()
    ...
    def get_mesh_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "load_shedder": self._load_shedder.get_stats(),
                "profiles": {
