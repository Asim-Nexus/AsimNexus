...
def run_all_tests():
    ...
    for method, endpoint, payload, desc in tests:
        if test_endpoint(method, endpoint, payload, desc):
            passed += 1
        else:
            failed += 1
    ...
    if test_endpoint("POST", "/api/universe/create", {
        ...
        # Get status
        if test_endpoint("GET", "/api/universe/test_api_user/status", None, "Universe status"):
            passed += 1
        else:
            failed += 1
            
        # Get lifecycle
        if test_endpoint("GET", "/api/universe/test_api_user/lifecycle", None, "Lifecycle"):
            passed += 1
        else:
            failed += 1
    else:
    ...
    # Test Universe Stats
    if test_endpoint("GET", "/api/universe/stats", None, "Universe stats"):
        passed += 1
    else:
        failed += 1
    ...
    if test_endpoint("POST", "/api/agent/mode/on", {
        "user_id": "test_api_user",
    ...
    if test_endpoint("GET", "/api/agent/status?user_id=test_api_user", None, "Agent status"):
        passed += 1
    else:
        failed += 1
    ...
    init_result = test_endpoint("POST", "/api/confirm/level3/initiate", {
    ...
    if test_endpoint("POST", "/api/sovereignty/check", {
        "country_code": "NP",
        "action": "test_action",
        "params": {"value": 100}
    }, "Compliance check"):
