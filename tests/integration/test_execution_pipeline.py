...
class TestFailureHandling:
    ...
    async def test_graceful_task_failure(self):
        ...
        async def failing_execute(task, deps=None):
            if task.id == "step_2":
                return ExecutionResult(task_id=task.id, success=False,
                    output=None, error="Simulated failure",
                    execution_time_ms=5
                )
            return ExecutionResult(task_id=task.id, success=True,
                output=f"Executed {task.type}",
        ...
        plan = TaskPlan(
            ...
            tasks=[
                Task(id="step_1", type="file_read", dependencies=[]),
                Task(id="step_2", type="risky_op", dependencies=["step_1"]),
                Task(id="step_3", type="file_write", dependencies=["step_2"]),
            ]
        )
    ...
    async def test_retry_mechanism(self):
        ...
        async def flaky_execute(task, deps=None):
            ...
            if attempts < 3:
                return ExecutionResult(task_id=task.id, success=False,
                    output=None, error="Temporary error",
                    execution_time_ms=5
                )
            return ExecutionResult(task_id=task.id, success=True,
        ...
        # Execute with retry
        result = await executor.execute(Task(id="test", type="file_read"))
    ...
    async def test_partial_failure_recovery(self):
        ...
        async def partial_execute(task, deps=None):
            ...
            success = task.id in ["step_1", "step_3"]
            return ExecutionResult(task_id=task.id, success=success,
        ...
        plan = TaskPlan(
            ...
            tasks=[
                Task(id="step_1", type="file_read", dependencies=[]),
                Task(id="step_2", type="process", dependencies=[]),  # Independent
                Task(id="step_3", type="file_write", dependencies=[]),  # Independent
            ],
        )
...
class TestMemoryManagement:
    ...
    async def test_result_accumulation_limits(self):
        ...
        plan = TaskPlan(
            ...
            tasks=[Task(id="step_1", type="process")]
        )
...
class TestExecutionTimeout:
    ...
    async def test_task_timeout(self):
        ...
        async def slow_execute(task, deps=None):
            ...
            return ExecutionResult(task_id=task.id, success=True, output="Done")
        
        executor.execute = AsyncMock(side_effect=slow_execute)
        
        task = Task(id="slow", type="file_read")
