...
async def test_uplink_connector():
    from core.depin.uplink_connector import UplinkConnector, UplinkNode
...
def test_uplink_node():
    from core.depin.uplink_connector import UplinkNode
    """Test Uplink node"""
    node = UplinkNode(id="n1", address="addr", bandwidth_capacity=1000.0)
...
def test_energy_device():
    from core.depin.daylight_connector import EnergyDevice, DeviceType
    """Test energy device"""
    device = EnergyDevice(id="d1", device_type=DeviceType.SOLAR_INVERTER, capacity_kwh=10.0)
