...
function LayerBadge({ icon, label, active }) {
  return (
    <div style={{
...
function VerdictBadge({ verdict }) {
  ...
  const icons = { allowed: '✅', vetoed: '🛑', pending_human: '⏳', rate_limited: '⏸' };
  return (
    <span style={{
...
function PendingCard({ call, onApprove, onReject, loading }) {
  return (
    <div style={{
...
export default function MCPPanel({ user }) {
  ...
  const load = useCallback(async () => {
    const [s, t, p, a] = await Promise.all([
      ...
      apiFetch('/api/mcp/pending'),
      apiFetch('/api/mcp/audit?limit=20'),
    ]);
    if (s) setStatus(s);
  ...
  useEffect(() => {
    const t = setInterval(async () => {
      const p = await apiFetch('/api/mcp/pending');
      if (p) setPending(p.pending || []);
    }, 5000);
  ...
  const handleApprove = async (callId) => {
    ...
    const r = await apiFetch(`/api/mcp/approve/${callId}`, { method: 'POST', body: '{}' });
    await load();
  ...
  const handleReject = async (callId) => {
    setActionLoading(true);
    await apiFetch(`/api/mcp/reject/${callId}`, { method: 'POST', body: '{}' });
    await load();
    setActionLoading(false);
  };
  ...
  const handleCallTool = async () => {
    ...
    try {
      ...
      const r = await apiFetch('/api/mcp/call', {
        method: 'POST',
        body: JSON.stringify({ tool_name: selectedTool, parameters: parsedParams }),
      });
      setCallResult(r);
      // Refresh audit
      const a = await apiFetch('/api/mcp/audit?limit=20');
      if (a) setAudit(a.audit || []);
      const p = await apiFetch('/api/mcp/pending');
      if (p) setPending(p.pending || []);
    } catch (e) {
      setCallResult({ error: `JSON parse error: ${e.message}` });
    }
    setCallLoading(false);
  };

  // Update default params when tool changes
  const toolParamDefaults = {
    file_list:       '{"path": "."}',
          ...
          <div style={{
            ...
            {callResult.output && (
              <pre style={{
                ...
                {JSON.stringify(callResult.output, null, 2).slice(0, 800)}
              </pre>
            )}
