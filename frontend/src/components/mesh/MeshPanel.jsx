...
export default function MeshPanel() {
  ...
  const load = useCallback(async () => {
    try {
      const [s, p] = await Promise.all([
        meshAPI.getStatus().then(r => r.data),
        meshAPI.getPeers().then(r => r.data),
      ]);
      setStatus(s);
      setPeers(p.peers || []);
    } catch { }
  ...
  const startDiscovery = async () => {
    setLoading(true);
    try {
      const r = await meshAPI.startDiscovery();
  ...
  const addPeer = async () => {
    ...
    setLoading(true);
    try {
      const r = await meshAPI.addPeer({ ip: addIp, port: parseInt(addPort) });
      if (r.data?.ip) { flash(`✅ Peer added: ${r.data.ip}`); setAddIp(''); await load(); }
    } catch (e) { flash(`❌ ${e.message}`); }
    finally { setLoading(false); }
  };

  const engageAirGap = async (level) => {
    const labels = { 1: 'Reduced', 2: 'LAN Only', 3: 'Isolated', 4: 'EMERGENCY' };
    if (level >= 3 && !window.confirm(`Engage ${labels[level]} air-gap? This will restrict network access.`)) return;
    setLoading(true);
    try {
      const r = await meshAPI.engageAirGap({ level, reason: 'Human-initiated' });
      if (r.data) { flash(`🔴 Air-gap: ${r.data.label}`); await load(); }
    } catch (e) { flash(`❌ ${e.message}`); }
    finally { setLoading(false); }
  };
  ...
  const disengage = async () => {
    setLoading(true);
    try {
      const r = await meshAPI.disengageAirGap();
