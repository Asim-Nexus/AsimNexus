/**
 * AsimOrbMaster.tsx - Production Floating Orb & Chat
 * Features: Mobile gestures, Themes (Dark/Light/Nepal/Dharma), Keyboard shortcuts, WebSocket, Position memory
 */
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { Mic, Activity, Radio, Eye, X, Maximize2, Moon, Sun, HelpCircle, Zap, Menu } from 'lucide-react';
...
const clamp = (v: number, m: number, M: number): number => Math.max(m, Math.min(M, v));
const zones = (w: number, h: number): { x: number; y: number }[] => [{ x: 8, y: 8 }, { x: w / 2 - ORB_SIZE / 2, y: 8 }, { x: w - ORB_SIZE - 8, y: 8 }, { x: 8, y: h / 2 - ORB_SIZE / 2 }, { x: w - ORB_SIZE - 8, y: h / 2 - ORB_SIZE / 2 }, { x: 8, y: h - ORB_SIZE - 8 }, { x: w / 2 - ORB_SIZE / 2, y: h - ORB_SIZE - 8 }, { x: w - ORB_SIZE - 8, y: h - ORB_SIZE - 8 }];

export default function AsimOrbMaster({ user: _user, onCommand: _onCommand, systemMetrics = {} }: AsimOrbMasterProps) {
    const [themeKey, setThemeKey] = useState<string>(() => { try { return localStorage.getItem('asimTheme') || 'dark'; } catch { return 'dark'; } });
    ...
    const [voiceData, setVoiceData] = useState<VoiceAnalysisData | undefined>(undefined);
    ...
    useEffect(() => { let a: number; const loop = () => { setOrbVis((p: { x: number; y: number }) => ({ x: lerp(p.x, orbPos.x, 0.18), y: lerp(p.y, orbPos.y, 0.18) })); setPopupVis((p: { x: number; y: number }) => ({ x: lerp(p.x, popupPos.x, 0.14), y: lerp(p.y, popupPos.y, 0.14) })); a = requestAnimationFrame(loop); }; a = requestAnimationFrame(loop); return () => cancelAnimationFrame(a); }, [orbPos, popupPos]);
    ...
    const toggleVoice = async () => {
        if (!isRecording) {
            if (await voiceAnalysisService.init()) {
                setIsRecording(true);
                setVisMode('spectrum');
                voiceAnalysisService.startAnalysis((a: VoiceAnalysisData) => setVoiceData(a));
            }
        } else {
    ...
    const snapOrb = useCallback((x: number, y: number) => {
        const z = zones(window.innerWidth, window.innerHeight);
        let b = { x, y };
        let bd = Infinity;
        z.forEach(p => {
            const d = Math.hypot(p.x - x, p.y - y);
            if (d < SNAP && d < bd) { bd = d; b = p; }
        });
        return b;
    }, []);
    ...
    useEffect(() => {
        const onKey = (e: KeyboardEvent) => {
            if (e.key === 'Escape') { setIsOpen(false); setShowHelp(false); setShowQuick(false); setIsRecording(false); }
            if (e.altKey && e.key === 'Enter') { e.preventDefault(); setIsOpen((o: boolean) => !o); }
            ...
            if (e.altKey && (e.key === '+' || e.key === '=')) { e.preventDefault(); setPopupSize((p: { width: number; height: number }) => ({ width: clamp(p.width + 40, MIN_W, MAX_W), height: clamp(p.height + 40, MIN_H, MAX_H) })); }
            if (e.altKey && e.key === '-') { e.preventDefault(); setPopupSize((p: { width: number; height: number }) => ({ width: clamp(p.width - 40, MIN_W, MAX_W), height: clamp(p.height - 40, MIN_H, MAX_H) })); }
            if (e.altKey && e.key === 'm') { e.preventDefault(); nextTheme(); }
            if (e.altKey && e.key === 'h') { e.preventDefault(); setShowHelp((h: boolean) => !h); }
        };
        window.addEventListener('keydown', onKey);
        return () => window.removeEventListener('keydown', onKey);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [isRecording, themeKey]);
    ...
    const handleQuick = (a: string) => {
        if (a === 'voice') toggleVoice();
        if (a === 'help') setShowHelp(true);
        if (a === 'fractal') setVisMode('fractal');
        if (a === 'wave') setVisMode('wave');
        if (a === 'spectrum') setVisMode('spectrum');
        if (a === 'close') { setIsOpen(false); setIsRecording(false); }
    };

    const handlePopupTouchStart = (e: React.TouchEvent) => { touchStartY.current = e.touches[0].clientY; };
    const handlePopupTouchMove = (e: React.TouchEvent) => {
        if (!isPopupDrag && !isResize && e.touches.length === 1) {
            const diff = touchStartY.current - e.touches[0].clientY;
            if (diff < -120) { setIsOpen(false); setIsRecording(false); }
        }
    };
        ...
        <>
            ...
            {!isOpen && (
                <div onMouseDown={(e) => { setIsOrbDrag(true); setOrbOff({ x: e.clientX - orbPos.x, y: e.clientY - orbPos.y }); }} onTouchStart={(e) => { const t = e.touches[0]; setIsOrbDrag(true); setOrbOff({ x: t.clientX - orbPos.x, y: t.clientY - orbPos.y }); }} onMouseEnter={() => setIsOrbHover(true)} onMouseLeave={() => setIsOrbHover(false)} onClick={() => { if (!isOrbDrag) { setIsOpen(true); setIsOrbHover(false); } }} style={{ position: 'fixed', left: orbVis.x, top: orbVis.y, width: ORB_SIZE, height: ORB_SIZE, borderRadius: '50%', background: `radial-gradient(circle at 30% 30%,${color}dd,${color}99)`, boxShadow: `0 0 ${20 + glow * 25}px ${color}88,0 0 ${50 + glow * 50}px ${color}44`, cursor: isOrbDrag ? 'grabbing' : 'grab', zIndex: 10000, transform: `scale(${orbScale})`, transition: isOrbDrag ? 'none' : 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1.275)', userSelect: 'none', WebkitUserSelect: 'none' }}>
                    ...
                    <img src="/asim-logo.png" alt="Asim Nexus" draggable={false} style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover', padding: 3 }} onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).src = '/ui/AsiM logo.png'; }} />
                    {systemMetrics.mesh_health && systemMetrics.mesh_health > 0.85 && <div style={{ position: 'absolute', bottom: 2, right: 2, width: 10, height: 10, borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 8px #22c55e', border: '2px solid rgba(10,10,26,0.9)' }} />}
            ...
            {isOpen && (
                <div onTouchStart={handlePopupTouchStart} onTouchMove={handlePopupTouchMove} style={{ position: 'fixed', left: popupVis.x, top: popupVis.y, width: popupSize.width, height: popupSize.height, background: theme.bg, borderRadius: 18, border: theme.border, boxShadow: theme.shadow, zIndex: 10000, display: 'flex', flexDirection: 'column', overflow: 'hidden', transition: isPopupDrag || isResize ? 'none' : 'width 0.25s ease,height 0.25s ease', cursor: isPopupDrag ? 'grabbing' : 'default' }}>
                    ...
                    <div style={{ flex: 1, overflow: 'hidden', minHeight: 0 }}><OmniChat /></div>
                    {['nw', 'ne', 'sw', 'se'].map(corner => {
                        const isL = corner.includes('w');
                        const isT = corner.includes('n');
                        const c = isT ? (isL ? 'nw-resize' : 'ne-resize') : (isL ? 'sw-resize' : 'se-resize');
                        return <div key={corner} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); setIsResize(true); setRc(corner); setRs({ x: e.clientX, y: e.clientY, w: popupSize.width, h: popupSize.height, px: popupPos.x, py: popupPos.y }); }} style={{ position: 'absolute', [isT ? 'top' : 'bottom']: 0, [isL ? 'left' : 'right']: 0, width: 16, height: 16, cursor: c, zIndex: 10003 }} title="Drag to resize" />;
                    })}
                </div>
            )}
            ...
            {showQuick && (
                <div onClick={() => setShowQuick(false)} style={{ position: 'fixed', inset: 0, zIndex: 10099, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(0,0,0,0.4)', padding: '0 20px 40px' }}>
                    <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 320, background: theme.bg, borderRadius: 18, border: theme.border, boxShadow: theme.shadow, padding: '12px 0' }}>
                        {[{ icon: Mic, label: 'Voice Input', a: 'voice' }, { icon: HelpCircle, label: 'Shortcuts', a: 'help' }, { icon: Eye, label: 'Fractal', a: 'fractal' }, { icon: Radio, label: 'Wave', a: 'wave' }, { icon: Activity, label: 'Spectrum', a: 'spectrum' }, { icon: X, label: 'Close', a: 'close' }].map((item, i) => {
                            const Icon = item.icon;
                            return <button key={i} onClick={() => { handleQuick(item.a); setShowQuick(false); }} style={{ display: 'flex', alignItems: 'center', gap: 14, width: '100%', padding: '12px 20px', border: 'none', background: 'none', color: theme.text, cursor: 'pointer', fontSize: 15, textAlign: 'left', borderBottom: i < 5 ? `1px solid ${theme.muted}22` : 'none' }}><Icon size={20} color={theme.primary} />{item.label}</button>;
                        })}
                    </div>
                </div>
            )}
