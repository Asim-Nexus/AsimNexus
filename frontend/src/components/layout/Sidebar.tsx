...
const THEMES: string[] = ['deep-space', 'aurora', 'government', 'corporate', 'medical', 'minimal'];
...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    const s: Record<string, React.CSSProperties | ((active: boolean) => React.CSSProperties)> = {
        ...
        section: { padding: '16px 20px 8px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: 2, opacity: 0.4, color: '#fff' } as React.CSSProperties,
        ...
        unBtn: (active: boolean): React.CSSProperties => ({
    ...
    return (
        <>
            ...
            <div style={s.sidebar as React.CSSProperties}>
                ...
                <div style={s.section as React.CSSProperties}>Navigation</div>
                {NAV_ITEMS.map(item => (
                    <div key={item.id} style={(s.navItem as (active: boolean) => React.CSSProperties)(activeRoute === item.id || activeRoute.startsWith(item.id + '/'))}
                ...
                <div style={s.section as React.CSSProperties}>Universe Mode</div>
                <div style={{ padding: '4px 20px' }}>
                    {UNIVERSES.map(u => (
                        <div key={u.id} style={(s.unBtn as (active: boolean) => React.CSSProperties)(universeMode === u.id)}
                ...
                <div style={s.section as React.CSSProperties}>Theme</div>
                <div style={{ padding: '4px 20px', display: 'flex', flexWrap: 'wrap' }}>
                    {THEMES.map(t => (
                        <button key={t} style={(s.themeBtn as (active: boolean) => React.CSSProperties)(theme === t)}
                ...
                <button style={s.logoutBtn as React.CSSProperties} onClick={onLogout}>↗ Logout</button>
            </div>
        </>
    );
};
