...
interface OnboardingPageProps {
    onComplete: (user: Record<string, unknown>, token: string) => void;
}
...
function simulateDeviceScan(): DeviceInfo[] {
    ...
    const hw: DeviceInfo[] = [
        ...
        { name: 'RAM', value: nav.deviceMemory ? `~${nav.deviceMemory} GB` : 'Unknown', status: '✅' },
        { name: 'GPU', value: 'WebGL Detected', status: '✅' },
        { name: 'Network', value: connection?.effectiveType ? (connection.effectiveType as string) : 'Unknown', status: '✅' },
    ...
    const handleRegister = async (): Promise<void> => {
        ...
        try {
            const res = await authAPI.register(
                ...
                null, 'NP', null
            ) as unknown as { success: boolean; user: Record<string, unknown>; token: string };
...
// ── Styles ──
const pageStyle: React.CSSProperties = { width: '100%', maxWidth: 520, margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'rgba(10,10,26,0.95)', color: '#fff' };
const h2Style: React.CSSProperties = { fontSize: '1.3rem', marginBottom: 20, fontWeight: 700 };
const btnPrimary: React.CSSProperties = { flex: 1, padding: '14px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#667eea,#764ba2)', color: '#fff', fontSize: '1rem', fontWeight: 600, cursor: 'pointer' };
const btnSecondary: React.CSSProperties = { padding: '14px 24px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.05)', color: '#fff', fontSize: '1rem', cursor: 'pointer' };
const langBtn: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 16, padding: 16, borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: '#fff', cursor: 'pointer' };
const langBtnActive: React.CSSProperties = { background: 'rgba(102,126,234,0.15)', borderColor: '#667eea' };
const roleBtn: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 16, padding: 16, borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: '#fff', cursor: 'pointer' };
const roleBtnActive: React.CSSProperties = { background: 'rgba(255,255,255,0.08)' };
const universeItem: React.CSSProperties = { padding: '10px 14px', borderRadius: 8, background: 'rgba(255,255,255,0.05)', fontSize: '0.9rem' };
