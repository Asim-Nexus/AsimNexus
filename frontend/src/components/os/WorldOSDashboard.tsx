...
import { worldOSAPI, svtAPI, consensusAPI } from '../../api/asimnexus';
...
interface QuadInfo {
    peer_count?: number;
    stats?: { total_peers?: number };
    trust_rules?: { description?: string };
    [key: string]: unknown;
}
...
interface DHTData {
    node_id?: string;
    routing?: { total_nodes?: number; occupied_buckets?: number };
    stored_keys?: number;
    [key: string]: unknown;
}
...
export default function WorldOSDashboard() {
    const [svt, setSvt] = useState<SVTData | null>(null);
    const [quad, setQuad] = useState<Record<string, QuadInfo> | null>(null);
    const [bugs, setBugs] = useState<BugStats | null>(null);
    const [cons, setCons] = useState<ConsStats | null>(null);
    ...
    const loadAll = useCallback(async () => {
        try {
            const [svtR, quadR, bugsR, consR, dhtR, fwR] = await Promise.allSettled([
                svtAPI.getStats().then(r => r.data),
                worldOSAPI.getQuadStatus().then(r => r.data),
                worldOSAPI.getBugStats().then(r => r.data),
                consensusAPI.getStatus().then(r => r.data),
                worldOSAPI.getDHTStatus().then(r => r.data),
                worldOSAPI.getFirewallStatus().then(r => r.data),
            ]);
            if (svtR.status === 'fulfilled') setSvt(svtR.value as unknown as SVTData);
            if (quadR.status === 'fulfilled') setQuad(quadR.value as unknown as Record<string, QuadInfo>);
            if (bugsR.status === 'fulfilled') setBugs(bugsR.value as unknown as BugStats);
            if (consR.status === 'fulfilled') setCons(consR.value as unknown as ConsStats);
            if (dhtR.status === 'fulfilled') setDht(dhtR.value as unknown as DHTData);
            if (fwR.status === 'fulfilled') setFw(fwR.value as unknown as FirewallData);
            setLoading(false);
        } catch { /* ignore */ }
    }, []);
        ...
        <div style={{ color: '#fff', fontFamily: 'system-ui,sans-serif', maxWidth: 1200 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                ...
                <div style={{ ...card(), borderColor: 'rgba(239,68,68,0.2)' }}>
                    ...
                    {bugs ? <>
                        <div style={{ fontSize: 22, fontWeight: 800, color: bugs.critical && bugs.critical > 0 ? '#ef4444' : '#34d399' }}>
                            {bugs.total} <span style={{ fontSize: 13, color: '#64748b' }}>bugs</span>
                        </div>
                        <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                            {bugs.critical && bugs.critical > 0 && <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 6, background: 'rgba(239,68,68,0.15)', color: '#f87171' }}>⛔ {bugs.critical} critical</span>}
                            {bugs.high && bugs.high > 0 && <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 6, background: 'rgba(251,146,60,0.15)', color: '#fb923c' }}>⚠️ {bugs.high} high</span>}
                            ...
                            {bugs.pending_human && bugs.pending_human > 0 && <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 6, background: 'rgba(167,139,250,0.15)', color: '#a78bfa' }}>👤 {bugs.pending_human} awaiting you</span>}
                        </div>
                ...
                <div style={{ ...card(), borderColor: 'rgba(167,139,250,0.2)' }}>
                    ...
                    {cons ? <>
                        ...
                        <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                            ...
                            {cons.pending_human && cons.pending_human > 0 && <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 6, background: 'rgba(251,146,60,0.15)', color: '#fb923c' }}>👤 {cons.pending_human} pending</span>}
                        </div>
