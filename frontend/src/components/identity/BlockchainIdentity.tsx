...
const BlockchainIdentity: React.FC<BlockchainIdentityProps> = ({ apiEndpoint: _apiEndpoint }) => {
    ...
    // Create DID
    const createDID = async (address: string) => {
        try {
            const response = await api.post('/api/blockchain/did', {
                public_key: address,
                network: selectedNetwork
            });

            const data = response.data as unknown as DIDResponse;
            setDid(data.did);
    ...
    const loadCredentials = async (did: string) => {
        try {
            const response = await api.get(`/api/blockchain/credentials/${did}`);
            const data = response.data as unknown as CredentialsResponse;
    ...
    // Load SBTs
    const loadSBTs = async (did: string) => {
        try {
            const response = await api.get(`/api/blockchain/sbts/${did}`);
            const data = response.data as unknown as SBTsResponse;
            setSbts(data.sbts || []);
        } catch (error) {
    ...
    const issueCredential = async (type: string) => {
        if (!did) return;

        setIsLoading(true);
        try {
            await api.post('/api/blockchain/credential', {
                issuer_did: did,
                subject_did: did,
    ...
    const createZKProof = async () => {
        ...
        setIsLoading(true);
        try {
            await api.post('/api/blockchain/zkproof', {
                prover_did: did,
    ...
    const formatDID = (did: string | null): string => {
        if (!did) return '';
