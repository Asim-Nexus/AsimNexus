...
interface SystemInfoData {
    ...
    cpu?: { count?: number; usage_percent?: number };
    memory?: { used_gb?: number; total_gb?: number; usage_percent?: number };
    disk?: { used_gb?: number; total_gb?: number; free_gb?: number; usage_percent?: number };
}
...
const LocalLLMChat: React.FC = () => {
    ...
    const loadModels = async () => {
        try {
            const response = await asimnexusAPI.getModels() as unknown as { data: { models?: ModelData[]; active_model?: string } };
            if (response.data) {
    ...
    const loadSystemInfo = async () => {
        try {
            const response = await asimnexusAPI.getSystemInfo() as unknown as { data: SystemInfoData };
            if (response.data) {
    ...
    const detectAction = (message: string): ActionData | null => {
    ...
    // Execute action
    const executeAction = async (action: ActionData): Promise<string> => {
        try {
            switch (action.type) {
                case 'create_folder': {
                    const folderResult = await asimnexusAPI.createFolder(action.data || 'new_folder') as unknown as { data?: { path?: string } };
                    return `✅ Folder "${action.data}" created successfully!\nPath: ${folderResult.data?.path || action.data}`;
                }

                case 'create_file': {
                    const fileResult = await asimnexusAPI.createFile(action.data || 'new_file.txt', '') as unknown as { data?: { path?: string } };
                    return `✅ File "${action.data}" created successfully!\nPath: ${fileResult.data?.path || action.data}`;
                }

                case 'list_directory': {
                    const dirResult = await asimnexusAPI.listDirectory(action.data || '.') as unknown as { data?: { items?: Array<{ type: string; name: string }> } };
                    const items = dirResult.data?.items || [];
                    const fileList = items.map((item: { type: string; name: string }) => `${item.type === 'directory' ? '📁' : '📄'} ${item.name}`).join('\n');
                    return `📂 Directory contents:\n${fileList || 'No items found'}`;
                }

                case 'system_info': {
                    const sysResult = await asimnexusAPI.getSystemInfo() as unknown as { data: SystemInfoData };
...
• Disk: ${sys?.disk?.used_gb}/${sys?.disk?.total_gb} GB (${sys?.disk?.usage_percent}% used)`;
            ...
            return `❌ Failed to execute action: ${err.response?.data?.detail || err.message}`;
        }
    };
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (action) {
                ...
                let response: { data?: { text?: string; model_used?: string; tokens_generated?: number; tokens?: number; generation_time_ms?: number; time_ms?: number } };

                // Use Gemma endpoint for Gemma model
                if (selectedModel === 'gemma-2-2b-it') {
                    response = await asimnexusAPI.generateGemma(inputMessage, 512) as unknown as { data?: { text?: string; model_used?: string; tokens_generated?: number; tokens?: number; generation_time_ms?: number; time_ms?: number } };
                } else {
                    // Use general endpoint for other models
                    response = await asimnexusAPI.generate(inputMessage, selectedModel, 512) as unknown as { data?: { text?: string; model_used?: string; tokens_generated?: number; tokens?: number; generation_time_ms?: number; time_ms?: number } };
                }
            ...
            setMessages(prev => [...prev, {
                id: Date.now() + 1,
                role: 'assistant',
                content: `❌ Error: ${err.response?.data?.detail || err.message || 'Failed to get response from local LLM.'}`,
                isError: true,
                timestamp: new Date().toLocaleTimeString()
            }]);
        } finally {
            ...
            <div className="quick-actions" style={{
                ...
                {quickActions.map((action, idx) => (
                    <button
                        ...
                        onMouseEnter={(e) => (e.target as HTMLButtonElement).style.background = '#3a3a6a'}
                        onMouseLeave={(e) => (e.target as HTMLButtonElement).style.background = '#2a2a4a'}
                    >
                        {action.label}
                    </button>
                ))}
