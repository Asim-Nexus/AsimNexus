...
import { personalAPI, chatAPI, consensusAPI, clonesAPI } from '../../api/asimnexus';
...
export default function WorldClones() {
    const [clones, setClones] = useState(CLONES_STATIC);
    ...
    const [cloneSpecs, setCloneSpecs] = useState(null);
    ...
    useEffect(() => {
        personalAPI.getClones()
            .then(r => {
                if (r?.data?.clones?.length) setClones(r.data.clones.map(mergeStatic));
            })
            .catch(() => { });

        consensusAPI.getStats()
            .then(r => { if (r?.data) setConsensusStats(r.data); })
            .catch(() => { });

        clonesAPI.getSpecs()
            .then(r => { if (r?.data?.clones) setCloneSpecs(r.data.clones); })
            .catch(() => { });
    }, []);
    ...
    function mergeStatic(apiClone) {
        const s = CLONES_STATIC.find(c => c.role === apiClone.role) || {};
        return { ...s, ...apiClone };
    }
    ...
    function toggleAgentMode() {
        ...
        if (newMode) {
            personalAPI.agentModeOn().catch(() => { });
        } else {
            personalAPI.agentModeOff().catch(() => { });
        }
    }
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {consensusStats && (
                        <div style={styles.consensusBadge}>
                            ...
                            {consensusStats.pending_human > 0 && (
...
function CloneCard({ clone, active, onClick }) {
