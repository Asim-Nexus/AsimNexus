...
export default function EconomyDashboardPanel({ user }) {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API}/api/marketplace/global-stats`, { headers: authHeaders() });
  ...
  const s = {
    ...
    cardTitle: { fontSize: '0.7rem', opacity: 0.5, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 },
  ...
  const renderModule = (title, data, fields) => {
      ...
      <div style={s.card}>
        ...
        {fields.map(([label, key]) => (
          <div key={key} style={s.statRow}>
            ...
            <span style={s.statVal}>{data[key] ?? '—'}</span>
          </div>
        ))}
      </div>
    );
