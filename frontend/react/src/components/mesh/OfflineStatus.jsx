 ...
 */
function computeHealth(meshStatus, airGapStatus) {
...
export default function OfflineStatus({ user }) {
    const [syncStatus, setSyncStatus] = useState(null);
    const [syncQueue, setSyncQueue] = useState([]);
    const [offlineStatus, setOfflineStatus] = useState(null);
    const [meshStatus, setMeshStatus] = useState(null);
    const [offlineCaps, setOfflineCaps] = useState(null);
    ...
    const loadAll = useCallback(async () => {
        try {
            ...
            if (syncRes.status === 'fulfilled') setSyncStatus(syncRes.value.data || syncRes.value);
            if (queueRes.status === 'fulfilled') {
                const data = queueRes.value.data || queueRes.value;
                setSyncQueue(Array.isArray(data) ? data : data.queue || data.items || []);
            }
            if (meshRes.status === 'fulfilled') setMeshStatus(meshRes.data || meshRes.value);
            if (capsRes.status === 'fulfilled') setOfflineCaps(capsRes.value.data || capsRes.value);

            // Also try user-specific offline status
            if (user?.id) {
                try {
                    const userOffline = await offlineAPI.getUserOfflineStatus(user.id);
                    setOfflineStatus(userOffline.data || userOffline);
                } catch {
    ...
    const handleRetrySync = async () => {
        ...
        try {
            ...
            flash(`❌ Sync retry failed: ${e.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const lastSync = syncStatus?.last_sync || offlineStatus?.last_sync || meshStatus?.last_sync;
