...
import { mcpAPI } from '../../api/odysseus';
import './odysseus.css';
...
function ServerCard({ server, onConnect, onDisconnect, onSelectTool }) {
...
function ToolDetail({ tool, onTryIt, onClose }) {
    return (
...
export default function MCPServiceBrowser({ user }) {
    ...
    const fetchStatus = useCallback(async () => {
        try {
            ...
            const serverStatuses = data?.servers || data || {};
            const merged = BUILT_IN_SERVERS.map((name) => ({
                name,
                status: serverStatuses[name]?.status || 'disconnected',
                tool_count: serverStatuses[name]?.tool_count || 0,
                tools: serverStatuses[name]?.tools || [],
                ...serverStatuses[name],
            }));
            setServers(merged);
            setError(null);
    ...
    const fetchTools = useCallback(async () => {
        ...
        try {
            ...
            setAllTools(Array.isArray(toolList) ? toolList : []);
        } catch {
            setAllTools([]);
        } finally {
    ...
    const handleConnect = useCallback(async (serverName) => {
        setServers((prev) =>
            prev.map((s) =>
        ...
        try {
            await mcpAPI.call(serverName, 'ping', {});
            setServers((prev) =>
                prev.map((s) =>
    ...
    const handleDisconnect = useCallback(async (serverName) => {
        setServers((prev) =>
            prev.map((s) =>
    ...
    const handleTryIt = useCallback(async (tool) => {
        if (!selectedServer) return;
        setTryLoading(true);
        setTryResult(null);
        try {
            const res = await mcpAPI.call(selectedServer.name, tool.name, {});
            setTryResult({
                success: true,
                data: res.data,
            });
            ...
            setTryResult({
                success: false,
                error: err.response?.data?.detail || err.message || 'Tool call failed',
            });
        } finally {
            setTryLoading(false);
        }
